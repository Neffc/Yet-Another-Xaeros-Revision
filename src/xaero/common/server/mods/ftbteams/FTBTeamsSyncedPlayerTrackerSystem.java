...
public class FTBTeamsSyncedPlayerTrackerSystem implements ISyncedPlayerTrackerSystem {
   ...
   public int getTrackingLevel(class_1657 tracker, class_1657 tracked) {
      if (FTBTeamsAPI.api().getManager().arePlayersInSameTeam(tracker.method_5667(), tracked.method_5667())) {
         ...
         Team trackerTeam = (Team)FTBTeamsAPI.api().getManager().getTeamForPlayerID(tracker.method_5667()).orElse(null);
         if (trackerTeam == null) {
            return 0;
         } else {
            Team trackedTeam = (Team)FTBTeamsAPI.api().getManager().getTeamForPlayerID(tracked.method_5667()).orElse(null);
            if (trackedTeam == null) {
               return 0;
            } else {
               return trackerTeam.getRankForPlayer(tracked.method_5667()) == TeamRank.ALLY
                     && trackedTeam.getRankForPlayer(tracker.method_5667()) == TeamRank.ALLY
                  ? 1
                  : 0;
            }
         }
