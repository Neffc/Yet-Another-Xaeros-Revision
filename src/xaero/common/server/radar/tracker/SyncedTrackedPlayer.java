...
public class SyncedTrackedPlayer {
   ...
   public boolean matchesEnough(class_1657 player, double maxAxisDistance) {
      return Math.abs(player.method_23317() - this.x) <= maxAxisDistance
         && Math.abs(player.method_23318() - this.y) <= maxAxisDistance
         && Math.abs(player.method_23321() - this.z) <= maxAxisDistance
         && player.method_37908().method_27983().method_29177().equals(this.dimension);
   }
